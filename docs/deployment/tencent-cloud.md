...
当前版本具有首次管理员初始化和登录保护；管理员及登录会话保存在宿主机 `/var/lib/zhangdan/zhangdan.sqlite`。上游配置和凭证仍只存在单个服务进程内存中，服务重启、服务器重启或再次发布后都需要重新录入。因此第一阶段仍采用单台腾讯云 Lighthouse、单个 Docker 容器和 SSH 隧道访问，不直接开放公网。
...
5. 候选容器使用临时认证数据卷，通过 `/healthz` 和 `/api/auth/status` 数据库初始化检查后再切换正式容器。
6. 正式容器将 `/var/lib/zhangdan` 挂载为可写认证数据卷，映射 `127.0.0.1:3210`，并设置 `restart: unless-stopped`。
...
不要运行多个实例、PM2 cluster 或多副本容器。更新或重启会保留管理员账号和未过期登录会话，但会清空内存中的上游账号和凭证。应提前记录非敏感连接参数，并在服务恢复后从可信浏览器重新录入凭证。不要删除或打包下载 `/var/lib/zhangdan/zhangdan.sqlite`；它包含管理员密码哈希和会话摘要。

公网域名上线前必须另行完成：

- HTTPS 证书与自动续期。
- 对登录和管理 API 做公网入口限速，并复核反向代理传递的客户端 IP。
- 凭证加密持久化和备份恢复。
- 服务端一分钟调度、并发锁和失败退避。
